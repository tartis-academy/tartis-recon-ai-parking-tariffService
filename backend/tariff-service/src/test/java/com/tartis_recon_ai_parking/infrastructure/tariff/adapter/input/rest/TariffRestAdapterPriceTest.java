...
class TariffRestAdapterPriceTest {
    ...
    void shouldReturnCalculatedPriceSuccessfully() throws Exception {
        ...
        TariffPriceRequest request = new TariffPriceRequest(tariffId, 120);
        PriceTransferDTO priceTransferDTO = new PriceTransferDTO(new BigDecimal("8.00"));

        when(priceCalculateUseCase.execute(tariffId, 120)).thenReturn(priceTransferDTO);
    ...
    void shouldReturnNotFoundWhenActiveTariffDoesNotExist() throws Exception {
        UUID tariffId = UUID.randomUUID();
        TariffPriceRequest request = new TariffPriceRequest(tariffId, 60);

        when(priceCalculateUseCase.execute(tariffId, 60))
